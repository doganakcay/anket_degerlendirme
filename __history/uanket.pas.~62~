...
begin
  SHowmessage('Anket Baþlýðý Girmediniz');
  exit;
end;
if tdanket_sayisi.Text='' then
begin
 SHowmessage('Aylýk Uygulanacak Asgari Anket Sayýsýný Giriniz');
 exit;
end;
if (CH.Checked=TRUE) AND (tdSoruSayisi.Text='')  then
BEGIN
  SHOWMESSAGE('Soru sayýsýný Giriniz ');
  exit;
END;
try
QGENEL_SORGU.Close;
...
BEGIN
  Showmessage('Anket deðerlendirmede kullanýldýðýndan silemezsiniz Önce Ýlgili Anketi Siliniz');
  exit;
END;
...
if QGENEL_SORGU.FieldByName('CEVAP').AsInteger>0 then
BEGIN
  Showmessage('Öncelikle Cevap Anahtarýný Siliniz');
  exit;
END;
...
if QGENEL_SORGU.FieldByName('CEVAP').AsInteger>0 then
BEGIN
  Showmessage('Öncelikle Ankete Ait Sorularý siliniz ');
  exit;
END;
end;
...
TRY
  ...
  begin
  showmessage('Cevap veya Puan Deðeri Girilmemiþtir  ');
  exit;
  end;
...
begin
  showmessage('Anket Tarihi Girilmemiþtir');
  exit;
end;

if QANKET.RecordCount<1 then
begin
  SHowmessage('Anket Seçilmemiþ');
  exit;
end;
if qsorular.RecordCount<1 then
begin
  SHowmessage('Ankete ait soru bulunamadý');
  exit;
end;
...
BEGIN
    ...
    BEGIN
      SHOWMESSAGE('Denek sayýsý girilmemiþ');
      exit;
    END;
